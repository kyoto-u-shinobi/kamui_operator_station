	...
	for(int i = 0; i < loop_count; i++)
	{
		for(int j = 0; j < ranges_size; j++)
		{
			if( clouds[i].points[j].z - origins[i].z > Z_MIN && 
				clouds[i].points[j].z - origins[i].z < Z_MAX &&
...
int main(int argc, char** argv)
{
	...
	ros::init(argc, argv, "laserscan_to_pointcloudgrid_gimbal_node");
	ros::NodeHandle n;

	double resolution = 0.05;
	if(ros::param::has("/laserscan_to_pointcloudgrid_gimbal_node/resolution"))
	{
		double resolution_;
		ros::param::get("/laserscan_to_pointcloudgrid_gimbal_node/resolution", resolution_);
		if(resolution_ > 0)
		{
			resolution = resolution_;
		}
	...
	int STOCK_SIZE = 40;
	if(ros::param::has("/laserscan_to_pointcloudgrid_gimbal_node/stock_size"))
	{
		int STOCK_SIZE_;
		ros::param::get("/laserscan_to_pointcloudgrid_gimbal_node/stock_size", STOCK_SIZE_);
		if(STOCK_SIZE_ > 0)
		{
			STOCK_SIZE = STOCK_SIZE_;
		}
